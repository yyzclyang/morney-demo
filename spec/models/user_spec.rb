...
RSpec.describe User, type: :model do
  ...
  it '创建 user 时 email 不能被占用' do
    ...
    user = User.create email: '1234@qq.com', password: '123456', password_confirmation: '123456'
    expect(user.errors.details[:email]).to include({:error => :taken, :value => '1234@qq.com'})
  end
  it '创建 user 时 email 的格式要对' do
    user = User.create email: '1234qq.com', password: '123456', password_confirmation: '123456'
    expect(user.errors.details[:email]).to include({:error => :invalid, :value => '1234qq.com'})
  end
  it '创建 user 时 password 必须存在' do
    user = User.create email: '1234@qq.com', password_confirmation: '123456'
    expect(user.errors.details[:password]).to include({:error => :blank})
  end
  it '创建 user 时 password 必须大于 6 位' do
    user = User.create email: '1234@qq.com', password: '12345', password_confirmation: '123456'
    expect(user.errors.details[:password]).to include({:count => 6, :error => :too_short})
  end
  it '创建 user 时 password_confirmation 必须存在' do
    user = User.create email: '1234@qq.com', password: '123456'
  ...
  it '创建 user 时 password_confirmation 必须与 password 相同' do
    user = User.create email: '1234@qq.com', password: '123456', password_confirmation: '123457'
